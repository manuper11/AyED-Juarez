...
//CAMBIAR POR UN METODO MAS EFECTIVO
...
void GestorBoveda::mostrar_cristal_mas_poderoso(bool& personaje, OptimusPrime& optimus, Megatron& megatron) {
    try {
        boveda.cristal_mas_poderoso(personaje,optimus,megatron);
    } catch (const ExcepcionBovedaCristales& e) {
        std::cout << "Error: " << e.what() << std::endl;
    }
}

void GestorBoveda::equipar_cristal(bool& personaje, OptimusPrime& optimus, Megatron& megatron) {
    try {
        boveda.equipar_cristal(personaje, optimus, megatron);
    } catch (const ExcepcionBovedaCristales& e) {
        std::cout << "Error: " << e.what() << std::endl;
    }
}